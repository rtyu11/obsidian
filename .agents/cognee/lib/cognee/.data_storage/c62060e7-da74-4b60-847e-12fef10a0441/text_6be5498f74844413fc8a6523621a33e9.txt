...
- `KindleInbox/` 内に `.agents/scripts/scribe_processed_log.txt` に記録されていないPDFファイルがあるか（ファイル名リネームではなくログで判定する）

**未処理PDFが1件以上ある場合：** `.agents/skills/scribe_inbox/SKILL.md` の手順をすべて実行する。

**KindleInboxが存在しない、または未処理PDFが0件の場合：** 「KindleInbox/：未処理PDFなし（スキップ）」と記録して次へ進む。
