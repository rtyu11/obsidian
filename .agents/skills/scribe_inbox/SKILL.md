...
Daily/Ideas/              ← アイデア・気づきメモの保存先
Daily/Tasks/              ← タスク・課題メモの保存先
Daily/Memo/               ← 雑多メモの保存先
...
| **アイデアメモ** | 新しいビジネスアイデア・企画・着想・発見 | `Daily/Ideas/` |
| **タスクメモ** | やること・課題・解決したい問題のリスト | `Daily/Tasks/` |
| **読書メモ** | 本のタイトルや著者名が明記、または本の内容についての気づき | `Notes/Books/` |
| **思考・考察ノート** | 深い分析・問いの探求・複数テーマにまたがる考察 | `Insights/` |
| **雑多メモ** | 上記いずれにも当てはまらないメモ | `Daily/Memo/` |
...
**追記フォーマット**（`アイデアリスト.md` / `課題リスト.md` / `メモリスト.md` の末尾）：
```markdown
### YYYY-MM-DD
- [メモの内容]
```

**独立ファイルフォーマット**（`Daily/Ideas/` / `Daily/Tasks/` / `Daily/Memo/` に保存）：
```markdown
---
date: YYYY-MM-DD
