...
#### C. Memo・Insightsとの関連付け
- `Memo/` と `Insights/` 全ファイルのタグを確認する
- 今回の本のキーワードタグと共通するファイルを検索する
- 共通タグがあれば：
  - 今回のBooksノートの `## 関連ノート` にそのファイルへのリンクを追記する
  - そのファイルにもBooksノートへのリンクを追記する（双方向）
- ※ テーマ横断メモが必要な場合は手動で `Insights/` にテーマ名.md として作成する
...
3. 以下を横断検索する：
   - `Insights/` — この本・関連タグに言及しているファイルを全件確認（テーマ横断メモを含む）
   - `Memo/` — 共通タグを持つファイルを全件確認

#### テーマ・タグについて話す場合
1. `Insights/` の全ファイルを確認し、テーマ名に一致するファイルを読む（日付なしのテーマ横断メモ）
2. 対応するファイルに紐づく各Booksノートの `ソース:` から Sourceファイルを読む
3. 以下を横断検索する：
...
- タグは必ず付与する（省略禁止）
- 関連本・関連Memo・関連Insightsが存在する場合はリンクを必ず記載する

**保存後：**
- 関連するBooksノートの `## 関連Insights` にリンクを追記する
- 関連するMemoノートにも追記する
- 関連するInsightsテーマ横断メモにも追記する
