...
var API_ENDPOINTS = {
  ...
  // Image endpoints
  IMAGES: (vaultId, userId) => {
    if (!vaultId || !userId) {
      throw new Error("vaultId\u3068userId\u306F\u5FC5\u9808\u30D1\u30E9\u30E1\u30FC\u30BF\u3067\u3059");
    }
    return `${BASE_URL}/images/${vaultId}/${userId}`;
  },
  IMAGE_CONTENT: (vaultId, userId, messageId) => {
...
var PAYMENT_PAGE_URL = process.env.PAYMENT_PAGE_URL || "https://line-notes-sync.pages.dev";
...
var MessageEncryptor = class {
   ...
   */
  async decryptImageData(encryptedData, metadata) {
...
var LinePlugin = class extends import_obsidian2.Plugin {
    ...
    try {
      ...
      if (newMessageCount > 0 || newImageCount > 0 || !isAutoSync) {
        ...
        if (newMessageCount > 0)
          parts.push(`${newMessageCount} new messages`);
        if (newImageCount > 0)
          parts.push(`${newImageCount} new images`);
        const summary = parts.length > 0 ? parts.join(", ") : "No new content";
        new import_obsidian2.Notice(`LINE sync completed. ${summary}.`);
      }
    } catch (err) {
    ...
    try {
      ...
      for (const image of images) {
        ...
        try {
          ...
          const contentResponse = await (0, import_obsidian3.requestUrl)({
            url: contentUrl,
            method: "GET"
          });
          ...
          const normalizedImageFolderPath = (0, import_obsidian2.normalizePath)(this.settings.imageFolderPath);
          ...
          const imageFilePath = (0, import_obsidian2.normalizePath)(`${this.settings.imageFolderPath}/${imageFileName}`);
  ...
  generateImageFileName(image, extension) {
  ...
  async createImageNote(image, imageFileName) {
    let noteFolderPath;
    if (this.settings.organizeByDate) {
    ...
    const normalizedNoteFolderPath = (0, import_obsidian2.normalizePath)(noteFolderPath);
    const noteFolder = this.app.vault.getAbstractFileByPath(normalizedNoteFolderPath);
    if (!noteFolder) {
      const normalizedBaseFolderPath = (0, import_obsidian2.normalizePath)(this.settings.noteFolderPath);
    ...
    if (this.settings.groupMessagesByDate) {
      ...
      const notePath = (0, import_obsidian2.normalizePath)(`${noteFolderPath}/${noteFileName}`);
      const content = [
        `---`,
        `source: LINE`,
  ...
  async updateImageSyncStatus(messageIds) {
    try {
      if (!this.settings.lineUserId) {
        return;
      }
...
var LineSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setHeading().setName("\u753B\u50CF\u8A2D\u5B9A");
    new import_obsidian2.Setting(containerEl).setName("Sync images").setDesc("LINE\u3067\u9001\u4FE1\u3057\u305F\u753B\u50CF\u3092\u540C\u671F\u3059\u308B\u304B\u3069\u3046\u304B").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncImages).onChange(async (value) => {
    ...
    const imageFolderSetting = new import_obsidian2.Setting(containerEl).setName("Image folder path").setDesc("\u753B\u50CF\u30D5\u30A1\u30A4\u30EB\u306E\u4FDD\u5B58\u5148\u30D5\u30A9\u30EB\u30C0").addText((text) => text.setPlaceholder("LINE/images").setValue(this.plugin.settings.imageFolderPath).onChange(async (value) => {
    ...
    const imageFileNameSetting = new import_obsidian2.Setting(containerEl).setName("Image file name template").setDesc("\u753B\u50CF\u30D5\u30A1\u30A4\u30EB\u306E\u547D\u540D\u30C6\u30F3\u30D7\u30EC\u30FC\u30C8\uFF08\u62E1\u5F35\u5B50\u306F\u81EA\u52D5\u3067\u4ED8\u4E0E\u3055\u308C\u307E\u3059\uFF09\n\u5229\u7528\u53EF\u80FD\u306A\u5909\u6570: {date}, {datecompact}, {time}, {datetime}, {messageId}, {userId}, {timestamp}").addText((text) => text.setPlaceholder("{date}-{messageId}").setValue(this.plugin.settings.imageFileNameTemplate).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setHeading().setName("\u30D7\u30EC\u30DF\u30A2\u30E0\u30D7\u30E9\u30F3");
    const statusText = this.getSubscriptionStatusText();
    const statusSetting = new import_obsidian2.Setting(containerEl).setName("\u73FE\u5728\u306E\u30D7\u30E9\u30F3").setDesc(statusText);
    if (this.plugin.settings.subscriptionStatus === "active") {
      statusSetting.addButton((button) => button.setButtonText("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u7BA1\u7406").onClick(() => {
        const userId = this.plugin.settings.lineUserId;
        const url = userId ? `${PAYMENT_PAGE_URL}/portal.html?userId=${userId}` : `${PAYMENT_PAGE_URL}/portal.html`;
        window.open(url, "_blank");
      }));
    } else {
      statusSetting.addButton((button) => button.setButtonText("\u30D7\u30EC\u30DF\u30A2\u30E0\u30D7\u30E9\u30F3\u306B\u30A2\u30C3\u30D7\u30B0\u30EC\u30FC\u30C9").setCta().onClick(() => {
        const userId = this.plugin.settings.lineUserId;
        const url = userId ? `${PAYMENT_PAGE_URL}?userId=${userId}` : PAYMENT_PAGE_URL;
        window.open(url, "_blank");
      }));
    }
    new import_obsidian2.Setting(containerEl).setName("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u3092\u66F4\u65B0").setDesc("\u30B5\u30FC\u30D0\u30FC\u304B\u3089\u6700\u65B0\u306E\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u3092\u53D6\u5F97\u3057\u307E\u3059").addButton((button) => button.setButtonText("\u66F4\u65B0").onClick(async () => {
      button.setDisabled(true);
      button.setButtonText("\u66F4\u65B0\u4E2D...");
      try {
        await this.plugin.fetchSubscriptionStatus();
        new import_obsidian2.Notice("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u3092\u66F4\u65B0\u3057\u307E\u3057\u305F");
        this.display();
      } catch (err) {
        new import_obsidian2.Notice("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u306E\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F");
      } finally {
        button.setDisabled(false);
        button.setButtonText("\u66F4\u65B0");
      }
    }));
    new import_obsidian2.Setting(containerEl).setHeading().setName("\u63A5\u7D9A\u8A2D\u5B9A");
